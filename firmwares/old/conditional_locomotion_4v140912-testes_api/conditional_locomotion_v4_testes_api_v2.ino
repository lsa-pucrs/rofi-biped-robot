...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void UpdateAction(){
  //currentAction->setRightTurnAction(); 
  //currentAction->setWalkAction();
 //currentAction->setRightTurnAction(); 
  if(currentAction->playbackEndCheck()){ //avoid changes in the middle of an animation     
    if(repeatTimes<=0)changeAction();
    else{
...
void changeAction(){
  ...
  while(Serial.available()){
      char c = Serial.read(); //get one bit from serial buffer
      readString += c; //makes the string readString
      delay(2); //slow looping to allow buffer to fill next char
    }
    if(readString.length() > 0){
       Serial.println(">>>");
       Serial.println(readString);
       //check if contain "spaces"
       int firstSpaceIndex = readString.indexOf(' ');//check if is has a "space" 
       //int lastSpaceIndex = readString.lastIndexOf('(');//check if is a command
       if(firstSpaceIndex>=0){//&&firstSpaceIndex!=lastSpaceIndex){
         /////////number compute
         String numberCommand = "";
         for(int i = firstSpaceIndex+1; i < readString.length(); i++){ //-1 nao le a ultima string
           numberCommand += readString[i];
         }
         int nC = numberCommand.toInt();
         /////////#number compute
         
         /////////name compute
         String nameCommand = "";
         for(int i = 0; i < firstSpaceIndex; i++){ //-1 nao le a ultima string
           nameCommand += readString[i];
         }
         /////////#name compute
         if(nameCommand=="walk"){
           currentAction->setWalkAction();
         ...
         else if(nameCommand=="stand"){
           Serial.print("Standing");
         }
         else if(nameCommand=="right"){
           currentAction->setRightTurnAction();
           repeatTimes = nC;
